...
8. 上传成功任务自动从队列清除；失败或取消任务保留并可从头重试，云端目录保持原始结构
...
账户信息直接位于设置页，不使用二级菜单或独立页面。页面元素：

- 头像
- GitHub 登录显示 GitHub 头像；纯数字 QQ 邮箱登录显示 QQ 头像，失败时回退默认头像
- GitHub 显示 GitHub 昵称；纯数字 QQ 邮箱登录后由后端自动获取 QQ 昵称，失败时显示“QQ 用户”而非 QQ 号
- 用户名
- 邮箱
- 角色标签
- 登录方式
