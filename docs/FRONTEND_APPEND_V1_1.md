...
管理员账号可通过系统文件选择器上传，可输入名称新建文件夹，并可重命名、移动、复制、删除云端文件或目录。重命名固定在原父目录通过 `MOVE` 完成，校验新名称且不允许覆盖同名目标；移动和复制通过云端目录选择器确定目标位置并保留原名称，不再要求手工输入完整路径。上传先写入同目录唯一临时名，完成后用 `MOVE` 提交；默认禁止覆盖，冲突后必须由管理员明确确认。普通用户不显示写操作，READ_ONLY 凭据也会在网络请求构造前拒绝写入。
