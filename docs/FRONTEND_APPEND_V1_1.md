...
下载 Room 仅持久化任务所有者、远程安全路径、进度、状态和续传校验信息；上传 Room 另行持久化任务所有者、批次、远端路径、SAF `content://` 来源 URI、进度和状态。WebDAV 地址、用户名、密码及短期解析 URL 均不进入数据库。下载前台服务按用户最多同时传输三个文件，超过限制的任务保持等待并在任一槽位释放后立即补位；文件先写入应用内部目录的 `.part`，仅在强 ETag 或原始 Last-Modified 匹配且 `206 Content-Range` 有效时追加，完成后通过 FileProvider 打开。

管理员账号可通过系统文件选择器一次选择一个或多个文件，也可选择完整文件夹，可输入名称新建文件夹，并可重命名、移动、复制、删除云端文件或目录。上传使用独立 Room 队列和前台服务，最多同时传输三个文件；超过限制的文件保持等待，任一文件完成、失败或取消后立即领取下一个。文件夹先展开为安全相对路径并按父级顺序创建远端目录，目录创建不占文件并发槽位，文件夹根目录、子目录、文件和空目录均保留；单个文件各占一个并发槽位。上传先写入同目录任务专属临时名，完成后用 `MOVE` 提交，成功任务在同一 Room 事务中自动从队列清除，失败和取消任务继续保留供重试或删除。默认禁止覆盖同名文件；远端已有同名目录时复用该目录并继续按原相对路径创建内容，取消或失败后的重试从零开始。SAF 只读授权可随未完成任务持久化，WebDAV 凭据仍仅驻留进程内存。重命名固定在原父目录通过 `MOVE` 完成，校验新名称且不允许覆盖同名目标；移动和复制通过云端目录选择器确定目标位置并保留原名称。复制先在目标父目录的唯一临时目录中执行原生 `COPY`，再通过 `MOVE` 提交；服务端对 `COPY` 返回兼容性错误时，客户端改用条件 `GET` 与流式 `PUT` 递归复制到新的临时目录，不写入本地持久存储，失败或取消只清理临时目录。普通用户不显示上传页及其他写操作，READ_ONLY 凭据也会在网络请求构造前拒绝写入。
